		...
		if (!empty($_POST['recup_mail'])) {
			...
			if (filter_var($recup_mail,FILTER_VALIDATE_EMAIL)){
				...
				if ($mailexist_count == 1){
			        ...
			        header("Location:http://127.0.0.1/Hexagon/mdp_oublie.php?section=code");
	...
	if (isset($_POST['verif_submit'],$_POST['verif_code'])){
		if(!empty($_POST['verif_code'])){
			...
			if ($verif_req == 1) {
				...
				header("Location:http://127.0.0.1/Hexagon/mdp_oublie.php?section=changemdp");
			}else{
				$error = "Code invalide";
			}
	...
	if (isset($_POST['change_submit'],$_POST['change_mdp'],$_POST['change_mdpc'])){
		...
		if (!empty($mdp) AND !empty($mdpc)){
			if ($mdp == $mdpc) {
				...
				header('Location:http://127.0.0.1/Hexagon/');
