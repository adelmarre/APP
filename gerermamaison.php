...
if (isset($_POST['supprimermaison'])) {
	...
	header("Location: maisonsallecapteur.php");//marchepas
}
	...
	<div class="colonnedroite">
		...
		<nav>
			<ul class="box_information">
			 ...
			 <li> 
			     <p> <?php echo '<a href="pagecontact.php?id='.$_SESSION['id'].'"  class="box">Nous contacter</a>'; ?>

			      <img src="https://us.123rf.com/450wm/imagecatalogue/imagecatalogue1610/imagecatalogue161000810/63481304-téléphone-vecteur-icône-le-style-de-l-image-est-un-gris-symbole-couleur-pictogramme-plat-.jpg" class="avatar_box">
			    </p>
			</li>
			...
			<?php
				...
				if (isset($_POST['ajouter'])) {
					...
					if ($pieceexist==0) {
						...
						$erreur= "Ce nom de pièce existe déjà";
					}
				if (isset($erreur)) 
			      { 
												...
												for ($i=1;$i<$longueurKey;$i++){
													...
													if (isset($_POST['choix']))
													{
														...
														$piece =$bdd->prepare('SELECT id_piece FROM piece  WHERE id_habitation= "'.$gethabitation.'" ');
                     									...
                     									while ($p = $piece -> fetch()) {
															$insertsecond= $bdd->prepare("INSERT INTO secondaire(id_principal, id_utilisateur_secondaire, id_piece, id_habitation) VALUES( ?, ?,?, ?)");
															$insertsecond->execute(array($getid,$usersecond['id'],$p['id_piece'], $gethabitation));
